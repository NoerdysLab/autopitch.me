  ...
  return (
    <div className="ai-stack">
      ...
      {/* `more` is always rendered so its 36px slot stays reserved — that
          ...
          just toggle the `visible` class to fade it in/out. */}
      {hasMore && (
        <button
          type="button"
          className={`more-btn ${moreReady && !showMore ? "visible" : ""}`}
          onClick={() => setShowMore(true)}
          aria-hidden={!moreReady || showMore}
          tabIndex={moreReady && !showMore ? 0 : -1}
        >
          more
        </button>
      )}
...
function AIBtn({
  ...
  // Tint the button's drop-shadow with the brand color so each AI button
  // gets a faint hint of its brand even at rest, blooming on hover.
  const tint = BRAND[platform].color;
  const style = {
    "--btn-shadow-rest": hexA(tint, 0.08),
    "--btn-shadow-hover": hexA(tint, 0.28),
...
// "#rrggbb" → "rgba(r, g, b, a)". Tiny helper, keeps the shadow tint
...
// Official brand SVG paths sourced from simple-icons (Claude, Perplexity,
// Gemini) and OpenAI's published brand assets (ChatGPT). Brand colors taken
// from each company's design guidelines.
const BRAND: Record<Platform, { color: string; path: string; gradient?: [string, string] }> = {
  claude: {
    color: "#D97757",
